...
import StructureInfoCard from "./DeckInfoCardStructure";

function Home() {
  const [decks, setDecks] = useState([]);
  const [loaded, setLoaded] = useState(false);

  //Loads deck information and causes a re-render when they load
  useEffect(() => {
    setDecks([]);
  ...
  //Deletes a deck and causes a re-render with the deleted deck removed
  ...
  //Adds a structure deck info card prior to loading
  if (!loaded) {
    for (let i = 0; i < 3; i++) {
